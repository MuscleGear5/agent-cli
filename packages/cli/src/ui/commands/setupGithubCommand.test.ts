...
describe('setupGithubCommand', async () => {
  ...
  it('returns a tool action to download github workflows and handles paths', async () => {
    ...
    const expectedSubstrings = [
      `set -eEuo pipefail`,
      `fakeOpenCommand "https://github.com/MuscleGear5/agent-cli-action`,
    ];
    ...
    if (gitignoreExists) {
      ...
      expect(gitignoreContent).toContain('.agent-cli/');
...
describe('updateGitignore', () => {
  ...
  it('does not add duplicate entries', async () => {
    const gitignorePath = path.join(scratchDir, '.gitignore');
    const existingContent = '.agent-cli/\nsome-other-file\ngha-creds-*.json\n';
    await fs.writeFile(gitignorePath, existingContent);
  ...
  it('adds only missing entries when some already exist', async () => {
    ...
    // Should not duplicate .agent-cli/ entry
  ...
  it('does not get confused by entries in comments or as substrings', async () => {
    ...
    const existingContent = [
      '# This is a comment mentioning .agent-cli/ folder',
      'my-app.agent-cli/config',
    ...
    const lines = content
      ...
      .filter((line) => line);
    expect(lines).toContain('.agent-cli/');
    expect(lines).toContain('gha-creds-*.json');
    expect(lines).toContain('my-app.agent-cli/config');
