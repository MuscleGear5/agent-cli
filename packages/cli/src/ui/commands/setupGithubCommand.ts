...
// Add Agent-Cli specific entries to .gitignore file
export async function updateGitignore(gitRepoRoot: string): Promise<void> {
  const gitignoreEntries = ['.agent-cli/', 'gha-creds-*.json'];
...
export const setupGithubCommand: SlashCommand = {
  ...
  action: async (
    ...
    const readmeUrl = `https://github.com/MuscleGear5/agent-cli-action/blob/${releaseTag}/README.md#quick-start`;
    ...
    for (const workflow of GITHUB_WORKFLOW_PATHS) {
      downloads.push(
        (async () => {
          const endpoint = `https://raw.githubusercontent.com/MuscleGear5/agent-cli-action/refs/tags/${releaseTag}/examples/workflows/${workflow}`;
