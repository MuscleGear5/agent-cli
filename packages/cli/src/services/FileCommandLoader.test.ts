...
import { Storage } from '@musclegear555/agent-cli-core';
...
vi.mock('@musclegear555/agent-cli-core', async (importOriginal) => {
  const original =
    await importOriginal<typeof import('@musclegear555/agent-cli-core')>();
  return {
    ...original,
  ...
  describe('Extension Command Loading', () => {
    ...
    it('only loads commands from active extensions', async () => {
      const extensionDir1 = path.join(
        process.cwd(),
        '.agent-cli/extensions/active-ext',
      );
      const extensionDir2 = path.join(
        process.cwd(),
        '.agent-cli/extensions/inactive-ext',
      );
    ...
    it('handles missing extension commands directory gracefully', async () => {
      const extensionDir = path.join(
        process.cwd(),
        '.agent-cli/extensions/no-commands',
      );
    ...
    it('handles nested command structure in extensions', async () => {
      const extensionDir = path.join(process.cwd(), '.agent-cli/extensions/a');
