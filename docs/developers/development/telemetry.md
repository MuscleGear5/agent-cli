...
Learn how to enable and setup OpenTelemetry for Agent-Cli.
...
observability framework — Agent-Cli's observability system provides:
...
All telemetry behavior is controlled through your `.agent-cli/settings.json` file.
...
1. Enable telemetry in your `.agent-cli/settings.json`:
   ```json
   {
     "telemetry": {
       "enabled": true,
   ...
   ```
2. Run Agent-Cli and send prompts.
...
1. Enable telemetry in your `.agent-cli/settings.json`:
   ```json
   {
     "telemetry": {
       ...
       "target": "local",
       "otlpEndpoint": "",
       "outfile": ".agent-cli/telemetry.log"
     }
   }
   ```
2. Run Agent-Cli and send prompts.
3. View logs and metrics in the specified file (e.g., `.agent-cli/telemetry.log`).
...
1. Run the automation script:
   ...
   - Save logs/metrics to `~/.agent-cli/tmp/<projectHash>/otel/collector.log`
...
- `agent-cli.file_operation`: This event occurs for each file operation.
  - **Attributes**:
    - `tool_name` (string)
...
- `agent-cli.api_request`: This event occurs when making a request to Qwen API.
  - **Attributes**:
    - `model`
...
- `agent-cli.api_response`: This event occurs upon receiving a response from Qwen API.
  - **Attributes**:
    - `model`
    - `status_code`
    - `duration_ms`
...
- `agent-cli.tool_output_truncated`: This event occurs when the output of a tool call is too large and gets truncated.
  - **Attributes**:
    - `tool_name` (string)
...
- `agent-cli.malformed_json_response`: This event occurs when a `generateJson` response from Qwen API cannot be parsed as a json.
  - **Attributes**:
    - `model`
...
- `agent-cli.extension_enable`: This event occurs when an extension is enabled
- `agent-cli.extension_install`: This event occurs when an extension is installed
  - **Attributes**:
    - `extension_name` (string)
    - `extension_version` (string)
    - `extension_source` (string)
    - `status` (string)
- `agent-cli.extension_uninstall`: This event occurs when an extension is uninstalled

### Metrics

Metrics are numerical measurements of behavior over time. The following metrics are collected for Agent-Cli (metric names remain `agent-cli.*` for compatibility):
