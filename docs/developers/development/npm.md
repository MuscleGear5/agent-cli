...
This monorepo contains two main packages: `@musclegear555/agent-cli` and `@musclegear555/agent-cli-core`.

## `@musclegear555/agent-cli`
...
When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@musclegear555/agent-cli-core`. This means that whether a user installs the package with `npm install -g @musclegear555/agent-cli` or runs it directly with `npx @musclegear555/agent-cli`, they are using this single, self-contained executable.

## `@musclegear555/agent-cli-core`
...
Releases are managed through the [release.yml](https://github.com/MuscleGear5/agent-cli/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
# Preview
npm install -g @musclegear555/agent-cli@preview

# Nightly
npm install -g @musclegear555/agent-cli@nightly
```
...
- `npx -y @musclegear555/agent-cli@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @musclegear555/agent-cli@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @musclegear555/agent-cli && npm uninstall -g @musclegear555/agent-cli && npm cache clean --force &&  npm install @musclegear555/agent-cli@<version>`
...
1.  Go to the [Actions tab](https://github.com/MuscleGear5/agent-cli/actions/workflows/release.yml) of the repository.
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @musclegear555/agent-cli`.
