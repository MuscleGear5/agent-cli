...
A minimum experimental TypeScript SDK for programmatic access to Agent-Cli.
...
- Node.js >= 20.0.0
- [Agent-Cli](https://github.com/MuscleGear5/agent-cli) >= 0.4.0 (stable) installed and accessible in PATH

> **Note for nvm users**: If you use nvm to manage Node.js versions, the SDK may not be able to auto-detect the Agent-Cli executable. You should explicitly set the `pathToQwenExecutable` option to the full path of the `qwen` binary.

## Quick Start

```typescript
import { query } from '@agent-cli/sdk';
...
Creates a new query session with the Agent-Cli.
...
| `env`                    | `Record<string, string>`                       | -                | Environment variables to pass to the Agent-Cli process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                              |
...
| `authType`               | `'openai' \| 'qwen-oauth'`                     | `'openai'`       | Authentication type for the AI service. Using `'qwen-oauth'` in SDK is not recommended as credentials are stored in `~/.agent-cli` and may need periodic refresh.                                                                                                                                                                                                                                                                                                                 |
...
```typescript
import { query, type SDKUserMessage } from '@agent-cli/sdk';
...
```typescript
import { query, type CanUseTool } from '@agent-cli/sdk';
...
```typescript
import { z } from 'zod';
import { query, tool, createSdkMcpServer } from '@agent-cli/sdk';
...
```typescript
import { query, isAbortError } from '@agent-cli/sdk';
...
```typescript
import { AbortError, isAbortError } from '@agent-cli/sdk';
