...
This document describes the `task` tool for Agent-Cli.
...
## How to use `task` with Agent-Cli
...
You can view available subagents by using the `/agents` command in Agent-Cli.
...
Subagents are configured through Agent-Cli's agent configuration system. Use the `/agents` command to:
