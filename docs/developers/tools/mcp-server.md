...
- **Stored securely** in `~/.agent-cli/mcp-oauth-tokens.json`
...
The `add` command configures a new MCP server in your `settings.json`. Based on the scope (`-s, --scope`), it will be added to either the user config `~/.agent-cli/settings.json` or the project config `.agent-cli/settings.json` file.
