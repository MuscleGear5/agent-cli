...
1. **DashScope** (Official, Free) - Automatically available for Agent-Cli Auth users (200 requests/minute, 2000 requests/day)
...
- **Agent-Cli Auth users:** DashScope is automatically added to your provider list, even if not explicitly configured
...
Pass API keys when running Agent-Cli:
...
**Note:** This setting requires a restart of Agent-Cli to take effect. Once disabled, the `web_search` tool will not be available to the model, even if web search providers are configured.
...
- **Cost:** Free
- **Authentication:** Automatically available when using Agent-Cli Auth authentication
- **Configuration:** No API key required, automatically added to provider list for Agent-Cli Auth users
- **Quota:** 200 requests/minute, 2000 requests/day
- **Best for:** General queries, always available as fallback for Agent-Cli Auth users
- **Auto-registration:** If you're using Agent-Cli Auth, DashScope is automatically added to your provider list even if you don't configure it explicitly
...
- **DashScope availability:** Automatically available for Agent-Cli Auth users, no configuration needed
...
- **For Agent-Cli Auth users:** The tool is automatically registered with DashScope provider, no configuration needed
...
- View your current settings in `~/.agent-cli/settings.json` (macOS/Linux) or `%USERPROFILE%\.agent-cli\settings.json` (Windows)
