...
```bash
git clone https://github.com/MuscleGear5/agent-cli.git # Or your fork's URL
cd agent-cli
```
...
To build both the `agent-cli` CLI utility and the sandbox container, run `build:all` from the root directory:
...
To start the Agent-Cli application from the source code (after building), run the following command from the root directory:

```bash
npm start
```

If you'd like to run the source build outside of the agent-cli folder, you can utilize `npm link path/to/agent-cli/packages/cli` (see: [docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) to run with `agent-cli`
...
The integration tests are designed to validate the end-to-end functionality of Agent-Cli. They are not run as part of the default `npm run test` command.
...
- `packages/`: Contains the individual sub-packages of the project.
  ...
  - `core/`: The core backend logic for Agent-Cli.
...
**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect agent-cli due to automatic exclusion. Use `.agent-cli/.env` files for agent-cli specific debug settings.

### React DevTools

To debug the CLI's React-based UI, you can use React DevTools. Ink, the library used for the CLI's interface, is compatible with React DevTools version 4.x.

1.  **Start the Agent-Cli application in development mode:**
